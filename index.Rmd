...
- **dyplr** (pour les fonctions `group_by()` et `mutate()`)
- **reshape2** (pour la fonction `reshape()`)
- **ggplot2**  (pour la fonction `ggplot()` utilisée pour générer les figures)
- **scales** (pour la fonction `scale_fill_distiller()` utilisée pour obtenir une palette de couleur particulière pour la figure)
